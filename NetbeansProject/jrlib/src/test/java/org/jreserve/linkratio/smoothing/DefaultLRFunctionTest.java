...
public class DefaultLRFunctionTest {
    ...
    public void testFit_Paid() {
        ...
        for(int d=0; d<lrs.length; d++)
            assertEquals(lrs[d], df.getValue(d), JRLibTestUtl.EPSILON);
        assertEquals(1d, df.getValue(lrs.length), JRLibTestUtl.EPSILON);
    }
    ...
    public void testGetValue() {
        for(int d=0; d<10; d++)
            assertEquals(1d, df.getValue(d), JRLibTestUtl.EPSILON);
    }
