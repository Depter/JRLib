package org.jreserve.gui.misc.officelaf.util;
    ...
    protected void paintSelectedBackground(Graphics2D g, int x, int y, int width, int height) {
        ...
        if (state == CommandButtonDisplayState.BIG) {
                if (!isSplit()) {
                    ...
                    Rectangle actionRect = new Rectangle(commandButton.getUI().getLayoutInfo().actionClickArea);
                    Rectangle popupRect = new Rectangle(commandButton.getUI().getLayoutInfo().popupClickArea);
