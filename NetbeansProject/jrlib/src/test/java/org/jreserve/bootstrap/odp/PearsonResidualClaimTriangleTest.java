...
public class PearsonResidualClaimTriangleTest {
    ...
    public final static double[][] PAID_EXPECTED = {
    ...
    public void testGetValue_Paid() {
        ...
        assertEquals(Double.NaN, paidResiduals.getValue(-1, 0), JRLibTestUtl.EPSILON);
        assertEquals(Double.NaN, paidResiduals.getValue(0, -1), JRLibTestUtl.EPSILON);
        for(int a=0; a<accidents; a++) {
            int devs = PAID_EXPECTED[a].length;
            assertEquals(Double.NaN, paidResiduals.getValue(a, -1), JRLibTestUtl.EPSILON);
            for(int d=0; d<devs; d++)
                assertEquals(PAID_EXPECTED[a][d], paidResiduals.getValue(a, d), JRLibTestUtl.EPSILON);
            assertEquals(Double.NaN, paidResiduals.getValue(a, devs), JRLibTestUtl.EPSILON);
        }
        assertEquals(Double.NaN, paidResiduals.getValue(accidents, 0), JRLibTestUtl.EPSILON);
    }
    ...
    public void testGetValue_Incurred() {
        ...
        assertEquals(Double.NaN, incurredResiduals.getValue(-1, 0), JRLibTestUtl.EPSILON);
        assertEquals(Double.NaN, incurredResiduals.getValue(0, -1), JRLibTestUtl.EPSILON);
        for(int a=0; a<accidents; a++) {
            int devs = INCURRED_EXPECTED[a].length;
            assertEquals(Double.NaN, incurredResiduals.getValue(a, -1), JRLibTestUtl.EPSILON);
            for(int d=0; d<devs; d++)
                assertEquals(INCURRED_EXPECTED[a][d], incurredResiduals.getValue(a, d), JRLibTestUtl.EPSILON);
            assertEquals(Double.NaN, incurredResiduals.getValue(a, devs), JRLibTestUtl.EPSILON);
        }
        assertEquals(Double.NaN, incurredResiduals.getValue(accidents, 0), JRLibTestUtl.EPSILON);
    }
}