...
public class ProjectFileProviderRegistrationProcessor extends AbstractRegistrationProcessor<ProjectFileProvider.Registration, ProjectFileProvider> {
    ...
    private final static String ERR_NO_PROJECT_NAME = 
        "Class '%s' annotated with '%s', but project name is not valid!";
    ...
    protected String getFileLocation(TypeElement element) throws LayerGenerationException {
        return String.format(LAYER_PATH, getProjectName(element))+getFileName(element);
    }

    private String getProjectName(TypeElement element) throws LayerGenerationException {
        ...
        if(project == null || project.trim().length()==0)
            throw new LayerGenerationException(String.format(ERR_NO_PROJECT_NAME, getClassName(element), annotationName()));
        return project;
    }
