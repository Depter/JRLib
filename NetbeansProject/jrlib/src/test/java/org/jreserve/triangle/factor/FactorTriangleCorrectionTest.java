...
import org.jreserve.triangle.factor.FactorTriangleCorrection;
...
public class FactorTriangleCorrectionTest {
    ...
    public void testGetCorrigatedValue() {
        assertEquals(CORRECTION, correction.getCorrigatedValue(), JRLibTestUtl.EPSILON);
    }
    ...
    public void testGetValue() {
        ...
        for(int a=-1; a<accidents; a++) {
            for(int d=-1; d<developments; d++) {
                ...
                assertEquals(expected, correction.getValue(a, d), JRLibTestUtl.EPSILON);
                assertEquals(sd, outsider.getValue(a, d), JRLibTestUtl.EPSILON);
            }
        }
    }
}