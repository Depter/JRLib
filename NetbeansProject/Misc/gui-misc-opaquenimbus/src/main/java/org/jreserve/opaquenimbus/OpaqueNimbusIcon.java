 ...
 * @author rbair
 */
class OpaqueNimbusIcon extends SynthIcon {
    private int width;
    private int height;
    private String prefix;
    private String key;

    OpaqueNimbusIcon(String prefix, String key, int w, int h) {
        this.width = w;
        this.height = h;
        this.prefix = prefix;
        this.key = key;
    ...
    public void paintIcon(SynthContext context, Graphics g, int x, int y,
        ...
        if (painter != null && context != null) {
            ...
            if (c instanceof JToolBar) {
                ...
                Object o = OpaqueNimbusLookAndFeel.resolveToolbarConstraint(toolbar);
    ...
    private int scale(SynthContext context, int size) {
        ...
        if (scaleKey != null) {
            if (OpaqueNimbusStyle.LARGE_KEY.equals(scaleKey)) {
                size *= OpaqueNimbusStyle.LARGE_SCALE;
            } else if (OpaqueNimbusStyle.SMALL_KEY.equals(scaleKey)) {
                size *= OpaqueNimbusStyle.SMALL_SCALE;
            } else if (OpaqueNimbusStyle.MINI_KEY.equals(scaleKey)) {
                // mini is not quite as small for icons as full mini is
                // just too tiny
                size *= OpaqueNimbusStyle.MINI_SCALE + 0.07;
            }
        }
        return size;
    }
