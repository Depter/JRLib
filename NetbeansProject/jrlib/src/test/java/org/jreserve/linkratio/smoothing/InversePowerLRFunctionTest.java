...
public class InversePowerLRFunctionTest {
    ...
    public void testFit_Paid() {
        ...
        ip.fit(lr);
        assertEquals( 0.10309578, ip.getA(), JRLibTestUtl.EPSILON);
        assertEquals(-1.87212371, ip.getB(), JRLibTestUtl.EPSILON);
    }
    ...
    public void testFit_Incurred() {
        ...
        ip.fit(lr);
        assertEquals( 0.04717668, ip.getA(), JRLibTestUtl.EPSILON);
        assertEquals(-1.35805726, ip.getB(), JRLibTestUtl.EPSILON);
    }

    @Test
    public void testGetA() {
        assertEquals(1d, ip.getA(), JRLibTestUtl.EPSILON);
    }

    @Test
    public void testGetB() {
        assertEquals(1d, ip.getB(), JRLibTestUtl.EPSILON);
    }

    @Test
    public void testGetValue() {
        for(int d=1; d<10; d++)
            assertEquals(1d+(double)d, ip.getValue(d-1), JRLibTestUtl.EPSILON);
        ...
        for(int d=0; d<EXPCETD_PAID.length; d++)
            assertEquals(EXPCETD_PAID[d], ip.getValue(d), JRLibTestUtl.EPSILON);
    }
