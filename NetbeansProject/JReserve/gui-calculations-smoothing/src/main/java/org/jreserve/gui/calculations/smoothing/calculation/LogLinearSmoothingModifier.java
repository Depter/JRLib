...
public abstract class LogLinearSmoothingModifier<C extends CalculationData> 
    extends AbstractRegressionSmoothingModifier<C> {

    public final static String ROOT_TAG = "logLinearRegressionSmoothing";

    public LogLinearSmoothingModifier(List<SmoothingCell> cells, Class<C> clazz, boolean hasIntercept) {
    ...
    protected final VectorSmoothing createVectorSmoothing() {
        return new AbstractSmoothing(
                getCellsAsIndices(), 
                new LogLinearRegressionSmoothingMethod(hasIntercept()));
    }
