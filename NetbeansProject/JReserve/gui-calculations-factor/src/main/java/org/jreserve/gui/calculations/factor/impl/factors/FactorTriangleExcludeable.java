 ...
 */
@Messages({
    "LBL.FactorTriangleExcludeable.Progress.Title=Excluding cells"
})
public class FactorTriangleExcludeable implements Excludeable {
    ...
    public boolean canExclude(Lookup context) {
        ...
        FactorTriangleCalculationImpl calculation = context.lookup(FactorTriangleCalculationImpl.class);
        if(calculation == null)
            return false;
    ...
    public void exclude(Lookup context) {
        ...
        String title = Bundle.LBL_FactorTriangleExcludeable_Progress_Title();
        TaskUtil.execute(task, null, title);
    }
    
    private class ExcludeTask implements Callable<Void> {
        
        private final UndoUtil<FactorTriangle> undo;
        ...
        private FactorTriangleExcludeModifier createModifier(Cell cell) {
            int a = cell.getAccident();
            int d = cell.getDevelopment();
            return new FactorTriangleExcludeModifier(a, d);
        }
    }    
}
