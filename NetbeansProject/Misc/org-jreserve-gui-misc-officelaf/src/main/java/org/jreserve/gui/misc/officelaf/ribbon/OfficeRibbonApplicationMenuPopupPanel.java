    ...
    public OfficeRibbonApplicationMenuPopupPanel(final JRibbonApplicationMenuButton button, RibbonApplicationMenu ribbonAppMenu) {
        ...
        this.setBorder(new LineBorder(BORDER_COLOR, BORDER_HIGHLIGHT1_COLOR, new Insets(14, 2, 0, 2)) {
            public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
                ...
                JRibbonApplicationMenuButton rendererButton = new JRibbonApplicationMenuButton(button.getRibbon());  // TODO flamingo 4.2 to 5.0 possible issue
        ...
        if (ribbonAppMenu != null) {
            final List<List<RibbonApplicationMenuEntryPrimary>> primaryEntries = ribbonAppMenu.getPrimaryEntries();
            
            /**
             * // TODO flamingo 4.2 to 5.0 changes.  Used to have a single list of values, fixme please
             */
            if(primaryEntries != null && primaryEntries.size() > 0) {
                for (final RibbonApplicationMenuEntryPrimary menuEntry : primaryEntries.get(0)) {
