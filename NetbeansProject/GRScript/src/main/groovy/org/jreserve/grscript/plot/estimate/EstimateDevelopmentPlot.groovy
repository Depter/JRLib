 ...
 * @version 1.0
 */
@PackageScope class EstimateDevelopmentPlot extends AbstractLinePlot {
    
    private Estimate estimate
    private double minValue = Double.NaN
    private double maxValue = Double.NaN
	
    EstimateDevelopmentPlot(Estimate estimate, PlotFormat format) {
        super(format)
    ...
    private void addAccident(DefaultCategoryDataset ds, int accident) {
        ...
        int obsDevs = estimate.getObservedDevelopmentCount(accident)
        PlotLabel obsKey = getObservedLabel(accident)
        
        for(int d=0; d<obsDevs; d++) {
            double v = estimate.getValue(accident, d)
            calculateBounds(v)
