...
import org.jreserve.linkratio.smoothing.SimpleLinkRatioSmoothing;
...
public class SimpleLinkRatioSmoothingTest {
    ...
    public void testGetValue() {
        assertEquals(Double.NaN, smoothing.getValue(-1), JRLibTestUtl.EPSILON);
        int length = EXPECTED.length;
        for(int d=0; d<length; d++)
            assertEquals("Ar d="+d, EXPECTED[d], smoothing.getValue(d), JRLibTestUtl.EPSILON);
        assertEquals(1d, smoothing.getValue(length), JRLibTestUtl.EPSILON);
    }
    ...
    public void testGetMackAlpha() {
        ...
        for(int d=0; d<DEVELOPMENTS; d++) {
            ...
            assertEquals(expected, smoothing.getMackAlpha(d), JRLibTestUtl.EPSILON);
        }
    }
}