...
@ExpandableElement.Registration(
    displayName = "#LBL.DevelopmentFactorPlotElement.Title",
    mimeType = FactorDataObject.MIME_TYPE,
    position = 400,
    prefferedID = "org.jreserve.gui.calculations.factor.editor.DevelopmentFactorPlotElement",
    background = "#COLOR.DevelopmentFactorPlotElement.Background",
    iconBase = "org/jreserve/gui/plot/icons/chart_line.png"
)
@Messages({
    "LBL.DevelopmentFactorPlotElement.Title=Development Periods",
    "COLOR.DevelopmentFactorPlotElement.Background=FF7D30"
})
public class DevelopmentFactorPlotElement extends AbstractExpandableElement {

    private FactorTriangleCalculation calculation;
    private InstanceContent ic = new InstanceContent();
    private ClipboardUtil.Copiable copiable;
    private Lookup lkp;
    private DevelopmentFactorPlotComponent panel;

    public DevelopmentFactorPlotElement() {
        this(Lookup.EMPTY);
    }
    
    public DevelopmentFactorPlotElement(Lookup context) {
    ...
    protected Component createVisualComponent() {
        if(panel == null) {
            panel = new DevelopmentFactorPlotComponent(calculation);
            initCopiable();
        }
        return panel;
    }
