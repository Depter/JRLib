...
class WidgetHeader extends JComponent {
    ...
    public WidgetHeader(TriangleWidget widget, int axis) {
        this.renderer = new DefaultWidgetHeaderRenderer(axis);
        this.widget = widget;
        this.axis = axis;
    ...
    private Dimension calculatePrefferedSize() {
        ...
        if (X_AXIS == axis) {
            return new Dimension(wSize.width, pSize);
        } else {
            return new Dimension(pSize, wSize.height);
        }
    }
    ...
    private void initPaint() {
        Component content = widget.getViewport();
        if(X_AXIS == axis) {
