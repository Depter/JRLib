...
@Smoothable.Registration(
    ...
    displayName = "#LBL.ArithmeticMAVectorSmoothable.Name",
    position = 300, 
    separatorBefore = true
)
@Messages({
    "LBL.ArithmeticMAVectorSmoothable.Name=Arithmetic MA"
})
public class ArithmeticMAVectorSmoothable extends AbstractMASmoothable<Vector> {
    ...
    protected SmoothDialogController<Vector> createController(Lookup context) {
        ...
        VectorTriangle triangle = (VectorTriangle) selection.getTriangle();
        Vector vector = triangle.getSource();
    ...
    static class DialogController extends MASmoothingDialogController<Vector> {
        ...
        public DialogController(Vector vector, TriangleGeometry geometry, List<Cell> cells) {
            super(vector, geometry, cells, Bundle.LBL_ArithmeticMAVectorSmoothable_Name());
        }
